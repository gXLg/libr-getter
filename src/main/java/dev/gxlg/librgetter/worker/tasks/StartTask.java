    ...
    public void work(TaskManager.TaskContext taskContext) throws StopTaskSignal {
        ...
        if (LibrGetter.config.safeChecker) {
            ...
            if (!taskContext.selectedVillager().hasVehicle()) {
                ...
                if (path != null) throw new TaskException("librgetter.unsafe");
            }
        }
        ...
        throw new StopTaskSignal(ctx -> {
            if (!LibrGetter.config.autoTool) ctx = ctx.withDefaultItem(player.getMainHandStack());
            if (resetCounter) ctx = ctx.withResetAttemptsCounter();

...
public class InventoryHelper {
    public static void selectItem(LocalPlayer player, int slot, MultiPlayerGameMode game, ClientNetwork clientNetwork) {
        Inventory inventory = player.getInventory();
        if (!Inventory.isHotbarSlot(slot)) {
            int syncId = player.getInventoryMenuField().getContainerIdField();
            int swap = inventory.getSuitableHotbarSlot();
            game.handleContainerInput(syncId, slot, swap, ContainerInput.SWAP(), player);
            slot = swap;
        }
